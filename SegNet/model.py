...
NUM_CLASSES = 2
NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN = 101926
NUM_EXAMPLES_PER_EPOCH_FOR_TEST = 50
...
def unpool(pool, ind, ksize=[1, 2, 2, 1], scope='unpool'):
    ...
    with tf.variable_scope(scope):
        ...
        if(ret.shape[1] == 46):
          return ret[:,:45,:,:] #hack bc of image size not divisible by 16(2^4)
        return ret
...
def inference(images, labels, batch_size, phase_train):
    ...
    upsample4 = unpool(pool4, pool4_indices, ksize=[1, 2, 2, 1])
    ...
    upsample3 = unpool(conv_decode4, pool3_indices, ksize=[1, 2, 2, 1])
    ...
    upsample2 = unpool(conv_decode3, pool2_indices, ksize=[1, 2, 2, 1])
    # decode 2
    ...
    upsample1 = unpool(conv_decode2, pool1_indices, ksize=[1, 2, 2, 1])
    # decode4
...
def test(FLAGS):
  ...
  output_dir = FLAGS.output_image_dir
  ...
  with tf.Session() as sess:
    ...
    for image_batch, label_batch  in zip(images, labels):
      ...
      if (FLAGS.save_image):
          base=os.path.basename(image_filenames[imageNum])        
          writeImage(im[0], output_dir + base) ##change directory here
          imageNum += 1
